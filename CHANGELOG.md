...
# [8.0.0-dismiss.2] (2025-10-21)

### New Features

- **android:** Added `localNotificationDismissed` event to track when users dismiss notifications
  - Listen for notification dismissals using `addListener('localNotificationDismissed', callback)`
  - Enables building features like persistent/reappearing notifications
  - Only fires for removable (non-repeating) notifications
- **TypeScript:** Added async function support to dismiss event listener type signature

### Performance Improvements

- **android:** Unscheduled notifications now appear instantly instead of with 100ms AlarmManager delay
  - Notifications without a `schedule` property bypass Android's AlarmManager
  - Significantly faster notification display (milliseconds vs 1-2 seconds)

### Breaking Changes

- **android:** Replaced manual JSON parsing with Gson for better performance and maintainability
- **android:** Removed custom `DateMatch` class in favor of standard Calendar-based scheduling  
- **android:** Improved `allowWhileIdle` alarm scheduling to use appropriate Android APIs

### Internal Improvements

- **android:** Converted `LocalNotification` and `LocalNotificationSchedule` to clean POJOs
- **android:** Simplified scheduling logic using `java.util.Calendar` instead of custom cron implementation
- **android:** Enhanced rescheduling logic in `TimedNotificationPublisher` to use Gson deserialization
- **android:** Added Gson 2.11.0 dependency for JSON serialization
- **android:** Added comprehensive test coverage for dismiss functionality (11 test cases)
- **android:** Implemented proper BroadcastReceiver for dismiss events with lifecycle management

### Notes

- Existing scheduled notifications created with previous versions will continue to work
- No JavaScript API changes required for existing functionality
- Schedule behavior remains functionally identical
- Dismiss event is Android-only (iOS doesn't provide this capability)

---------------------

# [8.0.0-dismiss.1] (2025-10-21)

### Initial Fork Release

- **android:** Forked from `@capacitor/local-notifications` v8.0.0-alpha.1
- **android:** Added dismiss event tracking foundation
- **android:** Implemented `NotificationDismissReceiver` for handling dismiss events
- **android:** Added comprehensive test coverage
