...
```bash
npm install ./path/to/local-notifications-withdismiss
npx cap sync
```

### JSON serialization

The Android implementation now uses Gson to deserialize notification payloads.
Keep schedules in ISO8601 (UTC) format and avoid undefined properties to
prevent surprises with Java default values.
...
## Dismiss Detection (Android Only)

This forked plugin adds support for detecting when notifications are dismissed by the user on Android. This enables real-time event-driven behavior without polling, such as automatically reappearing notifications.

### Using Dismiss Events
...
const handle = await LocalNotifications.addListener('localNotificationDismissed', (event) => {
  console.log('Notification dismissed:', event.notificationId);
  
  // Recreate the notification instantly (no schedule = immediate display)
  if (event.notificationId === REAPPEARING_NOTIFICATION_ID) {
    await LocalNotifications.schedule({
      notifications: [{
        id: REAPPEARING_NOTIFICATION_ID,
        title: 'Reappearing Notification',
        body: 'This notification reappears instantly when dismissed.',
        // No schedule = instant notification (appears in milliseconds)
      }]
    });
  }
});
...
- Each notification gets a `setDeleteIntent()` that fires when swiped away
- The `NotificationDismissReceiver` broadcasts the dismissal to the plugin
- The plugin emits the `localNotificationDismissed` event to JavaScript
- **Zero polling overhead** – entirely event-driven

### Performance Benefits

| Metric | Polling (old) | Event-driven (new) |
|--------|---------------|-------------------|
| Polling interval | 300ms | N/A |
| API calls | 2 every 300ms | 0 |
| Reappearance time | ~400ms | **<10ms** ⚡ |
| Notification display | 1-2 seconds (AlarmManager) | **Instant** (no schedule) |
| CPU/Battery | High | Minimal |

**Note:** Notifications without a `schedule` property display instantly, bypassing Android's AlarmManager for near-instantaneous appearance.
...
```ts
/// <reference types="@capacitor/local-notifications-dismiss" />
...
* [`addListener('localNotificationDismissed', ...)`](#addlistenerlocalnotificationdismissed-)
...
### addListener('localNotificationDismissed', ...)

```typescript
addListener(eventName: 'localNotificationDismissed', listenerFunc: (event: { notificationId: number; }) => void | Promise<void>) => Promise<PluginListenerHandle>
```

Listen for when a notification is dismissed by the user.

Only available on Android with the forked dismiss-aware plugin.

| Param              | Type                                                                                |
| ------------------ | ----------------------------------------------------------------------------------- |
| **`eventName`**    | <code>'localNotificationDismissed'</code>                                           |
| **`listenerFunc`** | <code>(event: { notificationId: number; }) =&gt; void \| Promise&lt;void&gt;</code> |

**Returns:** <code>Promise&lt;<a href="#pluginlistenerhandle">PluginListenerHandle</a>&gt;</code>

**Since:** 8.0.0-dismiss.1
